...
def challenges():
    ...
    records = {
        c.id: r.elapsed_time for r in recordsquery for c in challenges if r.challenge_id == c.id}
    if form.validate_on_submit():
        record = TimeRecord()
        ...
        challenge = Challenge.query.filter_by(
            id=form.challenge_id.data).first()
        if int(answer) == int(challenge.question.answer):
            db.session.add(record)
            db.session.commit()
            challenges = current_user.challenges.all()
            recordsquery = TimeRecord.query.filter_by(
